...
const AjouterBlog = () => {
	...
	const onSubmit = async (event) => {
		...
		try {
			const response = await fetch(`/api/ajouterBlog?id_utilisateur=${user.id_utilisateur}`, {
				method: 'POST',
				body: formData,
			});
				...
				<form className={styles.form} onSubmit={onSubmit}>
					<div className={`${styles.section} animate__animated animate__fadeIn`}>
						<label>
							...
							<input required type='text' placeholder='Titre du blog' name='titre' autoFocus />
							{/* {errors.titre && <p className={`${styles.erreur} animate__animated animate__headShake`}>{errors.titre.message}</p>} */}
						</label>
						<label>
							<span>Lien : </span>
							<input type='text' placeholder='Lien du blog' name='lien' />
						</label>
					</div>
					<div className={styles.section}>
						<label>
							<span className={styles.obligatoire}>Contenu : </span>
							<textarea required name='contenu' placeholder='Le contenu du blog ici...'></textarea>
